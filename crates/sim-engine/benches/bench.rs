...
use sim_engine::hospital::{HospitalParams, HospitalSimEngine};
...
fn bench_spatial_grid(c: &mut Criterion) {
    ...
    for &n in &[500usize, 1_000, 5_000] {
        ...
        // Build benchmark
        group.bench_with_input(BenchmarkId::new("build", n), &agents, |b, ags| {
            b.iter(|| {
        ...
        grid.build(&agents);
        group.bench_with_input(BenchmarkId::new("query_radius", n), &grid, |b, g| {
            b.iter(|| g.query_radius(40.0, 30.0, 5.0))
        });
    }
