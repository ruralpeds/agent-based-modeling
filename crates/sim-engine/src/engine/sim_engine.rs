...
use crate::engine::event_log::{EventLog, SimEvent, DeathCause};
use crate::decision_rules::{DecisionRule, RuleSetId, make_rule_set};
use crate::spatial::SpatialGrid;
use crate::rng::Mulberry32;
use crate::statistics::{SimStats, compute_stats};
...
impl SimEngine {
    ...
    pub fn step(&mut self) {
        ...
        let decisions: Vec<_> = self.agents.iter().map(|agent| {
            self.rule_set.decide(agent, &self.agents, &self.grid, &self.spatial, &self.params, &mut self.rng)
        }).collect();
        ...
        for (i, (agent, decision)) in self.agents.iter_mut().zip(decisions.iter()).enumerate() {
            ...
            // Prey: forage
            if agent.agent_type == AgentType::Prey && decision.action == crate::engine::agent::Action::Forage {
                let gained = self.grid.eat_at(agent.x, agent.y, self.params.prey_energy_gain);
            ...
            if agent.energy >= repro_threshold && self.rng.next_f32() < 0.02 {
                ...
                self.event_log.push(SimEvent::Birth { id, parent_id: agent.id, tick });
                births.push(offspring);
            }

            // Die from starvation or old age
            if agent.energy <= 0.0 || agent.age > 2000 {
                let cause = if agent.energy <= 0.0 { DeathCause::Starvation } else { DeathCause::OldAge };
                self.event_log.push(SimEvent::Death { id: agent.id, cause, tick });
                dead.push(i);
            }
        }
        ...
        for (pi, pj) in kill_pairs {
            ...
            self.agents[pi].energy = (self.agents[pi].energy + self.params.predator_energy_gain).min(self.params.max_energy);
            self.agents[pi].kills += 1;
            self.event_log.push(SimEvent::Kill { predator_id, prey_id, tick });
            extra_dead.push(pj);
        }
        ...
        // Grass regrowth
        self.grid.regrow(self.params.grass_regrow_rate, &mut self.rng);
    ...
    pub fn prey_count(&self)  -> usize { self.agents.iter().filter(|a| a.agent_type == AgentType::Prey).count() }
    pub fn pred_count(&self)  -> usize { self.agents.iter().filter(|a| a.agent_type == AgentType::Predator).count() }
