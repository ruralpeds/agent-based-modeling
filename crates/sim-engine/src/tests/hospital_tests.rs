...
mod tests {
    ...
    fn stats_always_valid() {
        ...
        for _ in 0..50 {
            ...
            assert!(
                s.cumulative_hai <= s.census as u32 + 5_000,
                "impossible HAI count"
            );
            assert!(
                s.hh_compliance_rate >= 0.0 && s.hh_compliance_rate <= 1.0,
                "HH rate out of [0,1]: {}",
                s.hh_compliance_rate
            );
            let state_sum: u32 = s.disease_state_counts.iter().sum();
            assert_eq!(
                state_sum, s.census as u32,
                "disease state counts {} != census {}",
                state_sum, s.census
            );
        }
    }
    ...
    fn census_never_exceeds_max_patients() {
        ...
        assert!(
            engine.patient_count() <= 500,
            "census exceeded MAX_PATIENTS"
        );
    }
    ...
    fn sprt_log_lr_changes_with_cases() {
        ...
        assert!(
            signalled,
            "SPRT should signal outbreak after injecting 5 cases/pt-day × 100 intervals"
        );
    }
    ...
    fn reset_clears_state() {
        ...
        assert_eq!(
            engine.cumulative_hai(),
            0,
            "HAI count should be 0 after reset"
        );
    }
    ...
    fn mean_wbc_in_physiological_range() {
        ...
        if s.census > 0 {
            assert!(
                s.mean_wbc >= 1.0 && s.mean_wbc <= 25.0,
                "mean WBC {:.2} outside physiological range [1, 25]",
                s.mean_wbc
            );
            assert!(
                s.mean_temperature >= 35.0 && s.mean_temperature <= 42.0,
                "mean temperature {:.2} outside physiological range",
                s.mean_temperature
            );
        }
    }
