...
mod tests {
    ...
    fn constant_rate_mean_inter_arrival() {
        ...
        let nhpp = NhppArrivalProcess::new(
            base_rate,
            [1.0f32; 24],
            [1.0f32; 7],
            1.0,
    ...
    fn rate_at_reflects_hourly_factor() {
        ...
        assert!(r15 > r3, "Afternoon rate {r15:.3} should exceed overnight rate {r3:.3}");
        assert!(r15 > 0.0);
        assert!(r3  > 0.0);
    }
    ...
    fn ed_default_peak_rate_in_range() {
        ...
        assert!(r > 3.0 && r < 6.0, "Peak ED rate {r:.3} outside plausible range [3, 6] patients/hr");
    }
}
