...
mod tests {
    ...
    use crate::stochastic::survival::{
        cox_hazard_ratio, CoxCovariates, CoxParams, SurvivalDistribution,
    };
    ...
    fn exponential_mean_within_5pct() {
        ...
        assert!(
            rel_err < 0.05,
            "Exp mean {mean:.3} vs theoretical {expected:.3}"
        );
    }
    ...
    fn weibull_icu_los_mean_within_5pct() {
        ...
        assert!(
            mean_err < 0.05,
            "Weibull mean {mean:.2} vs theoretical {exp_mean:.2}"
        );
        assert!(
            var_err < 0.10,
            "Weibull var  {var:.2}  vs theoretical {exp_var:.2}"
        );
    }
    ...
    fn lognormal_ward_los_mean_within_5pct() {
        ...
        assert!(
            rel_err < 0.05,
            "LogNormal mean {mean:.3} vs theoretical {exp_mean:.3}"
        );
    }
    ...
    fn lognormal_ed_los_mean_within_5pct() {
        ...
        assert!(
            rel_err < 0.05,
            "ED LOS mean {mean:.3} vs theoretical {exp_mean:.3}"
        );
    }
    ...
    fn cox_hr_increases_with_comorbidity() {
        ...
        assert!(
            hr_high > hr_low,
            "High-risk HR {hr_high:.3} should exceed low-risk HR {hr_low:.3}"
        );
    ...
    fn cox_adjusted_sample_shorter_for_high_hr() {
        ...
        let params = CoxParams::default();
        let low_cov = CoxCovariates {
            age: 45.0,
            ..CoxCovariates::default()
        };
        let high_cov = CoxCovariates {
            age: 80.0,
            charlson_index: 8.0,
            admission_severity: 15.0,
            ventilated: true,
            central_line: true,
        };
        ...
        let mean_low: f32 = (0..n)
            .map(|_| dist.sample_adjusted(hr_low, &mut rng))
            .sum::<f32>()
            / n as f32;
        let mut rng2 = Mulberry32::new(5);
        let mean_high: f32 = (0..n)
            .map(|_| dist.sample_adjusted(hr_high, &mut rng2))
            .sum::<f32>()
            / n as f32;

        assert!(
            mean_high < mean_low,
            "High-risk mean LOS {mean_high:.2} should be < low-risk {mean_low:.2}"
        );
    }
}
