use super::types::{HospitalUnit, ResistanceProfile, Sex, TreatmentAction};
...
impl PatientAgent {
    ...
    pub fn new_icu(
        ...
        let cov = CoxCovariates {
            age: age as f32,
            charlson_index: charlson as f32,
        ...
        Self {
            ...
            drug_conc: 0.0,
            immunity_score,
            pathogen_load: if disease_state == DiseaseState::Colonized {
                0.3
            } else {
                0.0
            },
    ...
    pub fn immunosuppression_factor(&self) -> f32 {
        (1.0 + 0.15 * self.charlson_index as f32 / 10.0
    ...
    pub fn mortality_multiplier(&self) -> f32 {
        (1.0 + 0.05 * (self.age as f32 - 65.0).max(0.0)
    ...
    pub fn antibiotic_days_f32(&self) -> f32 {
        self.antibiotic_days as f32
    }

    /// Current treatment action (for buffer serialisation).
    pub fn treatment_action(&self) -> TreatmentAction {
